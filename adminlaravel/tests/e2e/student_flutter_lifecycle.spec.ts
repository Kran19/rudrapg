...
test.describe('Real Flutter Web + Sub Admin E2E Lifecycle & Negative Audits', () => {
  ...
  test('1. Flutter Web Client Loading & Accessibility Check', async ({ page }) => {
    ...
    await page.goto('http://127.0.0.1:8088/login', { waitUntil: 'domcontentloaded' });
  ...
  test('2. Negative & Boundary Audits (Invalid Inputs & Access Bounds)', async ({ request }) => {
    ...
    const invalidPhoneRes = await request.post('http://127.0.0.1:8088/api/v1/student/register', {
    ...
    const unauthRes = await request.get('http://127.0.0.1:8088/api/v1/student/profile', {
      headers: { 'Accept': 'application/json' }
    });
    expect(unauthRes.status()).toBe(401);

    // 2c. Student Attempting Sub-Admin Endpoints (401/403 expected)
    const forbiddenRes = await request.get('http://127.0.0.1:8088/api/v1/sub-admin/pending-verifications', {
      headers: { 'Accept': 'application/json' }
    });
  ...
  test('4. Complete Master 5-Stage Onboarding Chain Execution', async ({ request }) => {
    ...
    const subAdminLoginRes = await request.post('http://127.0.0.1:8088/api/v1/auth/login', {
      headers: { 'Accept': 'application/json' },
    ...
    const kycRes = await request.post(`http://127.0.0.1:8088/sub-admin/verifications/${registeredStudentId}/approve-kyc`, {
      headers: { 'Accept': 'application/json' }
    });
    ...
    const bedRes = await request.post(`http://127.0.0.1:8088/api/v1/sub-admin/registrations/${registeredStudentId}/approve`, {
    ...
    const profStage3 = await request.get('http://127.0.0.1:8088/api/v1/student/profile', {
    ...
    const finalProfileRes = await request.get('http://127.0.0.1:8088/api/v1/student/profile', {
      headers: { 'Authorization': `Bearer ${studentToken}`, 'Accept': 'application/json' }
    });
